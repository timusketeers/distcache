...
public class DistributedQueue {
	...
	public DistributedQueue(ZooKeeper zookeeper, String dir, List<ACL> acl) {
		this.dir = dir;
		try {
			new ZookeeperClient(zookeeper).ensureExists(dir, null, CreateMode.PERSISTENT);
		} catch (Exception e) {
		...
		for (String childName : childNames) {
			try {
				...
				if (!childName.regionMatches(0, prefix, 0, prefix.length())) {
					logger.debug("Found child node with improper name: " + childName);
					continue;
				}
				...
				logger.warn("Found child node with improper format : " + childName, e);
			}
		}
	...
	private QueueEvent element() throws NoSuchElementException, KeeperException, InterruptedException {
				...
				if (headNode != null) {
					try {
						return new QueueEvent(dir + "/" + headNode,
								zookeeper.getData(dir + "/" + headNode, null, null), null);
					} catch (KeeperException.NoNodeException e) {
						logger.error("Exception", e);
					}
				}
	...
	private class LatchChildWatcher implements Watcher {
		...
		public void process(WatchedEvent event) {
			logger.info("Watcher fired on path: " + event.getPath() + " state: " + event.getState() + " type "
					+ event.getType());
			synchronized (lock) {
				this.event = event;
				lock.notifyAll();
	...
	public boolean offer(byte[] data) throws KeeperException, InterruptedException {
		return createData(dir + "/" + prefix, data, CreateMode.PERSISTENT_SEQUENTIAL) != null;
	}
	...
	public QueueEvent offer(byte[] data, long timeout) throws KeeperException, InterruptedException {
		...
		String watchID = createData(dir + "/" + response_prefix + path.substring(path.lastIndexOf("-") + 1), null,
